...
model.startup<- lm(Profit~ R.D.Spend+ Administration + Marketing.Spend + State , data = startup) # lm(Y ~ X)
summary(model.startup)

model.startupR<- lm(Profit ~ R.D.Spend)
summary(model.startupR)
...
model.startup1 <- lm(Profit ~ R.D.Spend + Administration + Marketing.Spend +State, data = startup[-46, ])
...
VIFR <- lm(R.D.Spend ~ Administration + Marketing.Spend + State , data = startup)
VIFA <- lm(Administration ~ R.D.Spend + Marketing.Spend + State, data = startup)
VIFM <- lm(Marketing.Spend ~ R.D.Spend + Administration + State, data = startup)
VIFS <- lm(State ~ R.D.Spend + Administration + Marketing.Spend , data = startup)
...
model.final1 <- lm(Profit ~ R.D.Spend + Administration + Marketing.Spend +State, data = startup[-46, ])
...
# Load the Cars dataset
library(readr)
car_data <- read.csv(file.choose(), header = TRUE )
View(car_data)
...
# Load the Cars dataset
library(readr)
avacado_data <- read.csv(file.choose(), header = TRUE )
View(avacado_data)
